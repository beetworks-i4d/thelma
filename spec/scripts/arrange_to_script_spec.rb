...
RSpec.describe 'arrange_to_script.rb' do
  ...
  describe 'tree format script' do
    let(:tree_beats) do
      [
        { 'id' => 'hook', 'role' => 'hook', 'label' => 'Hook',
          'text' => 'Opening hook.', 'parent' => nil, 'children' => [] },
        { 'id' => 'intro', 'role' => 'section', 'label' => 'Introduction',
          'text' => 'Intro text.', 'parent' => nil, 'children' => ['bb_1', 'bb_2'] },
        { 'id' => 'bb_1', 'role' => 'blueprint', 'label' => 'BB #1 — First idea',
          'text' => 'First idea details.', 'parent' => 'intro', 'children' => [] },
        { 'id' => 'bb_2', 'role' => 'blueprint', 'label' => 'BB #2 — Second idea',
          'text' => 'Second idea details.', 'parent' => 'intro', 'children' => [] },
        { 'id' => 'end_cta', 'role' => 'cta', 'label' => 'END CTA',
          'text' => 'Closing words.', 'parent' => nil, 'children' => [] }
      ]
    end

    it 'resolves --short by exact id' do
      Dir.mktmpdir do |dir|
        lib_dir = make_arrange_library(dir, short_beats: tree_beats, script_format: 'tree')
        response = {
          'beats' => [
            { 'beat_id' => 'bb_1', 'type' => 'blueprint', 'script_text' => 'First idea details.',
              'clips' => [{ 'source' => 'video.mp4', 't_in' => 1.0, 't_out' => 2.0,
                            'transcript_match' => 'test', 'take_id' => nil, 'notes' => nil }] }
          ]
        }
        seed_llm_response(lib_dir, 'bb_1', response)
        result = run_arrange(dir, short_id: 'bb_1')

        expect(result[:exit_code]).to eq(0)
        expect(result[:stderr]).to include('leaf')
        expect(result[:arrangement]['beats'].size).to eq(1)
      end
    end

    it 'resolves --short by label substring' do
      Dir.mktmpdir do |dir|
        lib_dir = make_arrange_library(dir, short_beats: tree_beats, script_format: 'tree')
        response = {
          'beats' => [
            { 'beat_id' => 'bb_2', 'type' => 'blueprint', 'script_text' => 'Second idea details.',
              'clips' => [{ 'source' => 'video.mp4', 't_in' => 3.0, 't_out' => 4.0,
                            'transcript_match' => 'test', 'take_id' => nil, 'notes' => nil }] }
          ]
        }
        seed_llm_response(lib_dir, 'Second idea', response)
        result = run_arrange(dir, short_id: 'Second idea')
    ...
    it 'includes children when resolving a parent beat' do
      Dir.mktmpdir do |dir|
        lib_dir = make_arrange_library(dir, short_beats: tree_beats, script_format: 'tree')
        # When targeting 'intro', should get intro + bb_1 + bb_2 = 3 beats
        response = {
          'beats' => [
            { 'beat_id' => 'intro', 'type' => 'section', 'script_text' => 'Intro text.',
              'clips' => [{ 'source' => 'video.mp4', 't_in' => 1.0, 't_out' => 2.0,
                            'transcript_match' => 'test', 'take_id' => nil, 'notes' => nil }] },
            { 'beat_id' => 'bb_1', 'type' => 'blueprint', 'script_text' => 'First idea details.',
              'clips' => [{ 'source' => 'video.mp4', 't_in' => 2.5, 't_out' => 3.5,
                            'transcript_match' => 'test', 'take_id' => nil, 'notes' => nil }] },
            { 'beat_id' => 'bb_2', 'type' => 'blueprint', 'script_text' => 'Second idea details.',
              'clips' => [{ 'source' => 'video.mp4', 't_in' => 4.0, 't_out' => 5.0,
                            'transcript_match' => 'test', 'take_id' => nil, 'notes' => nil }] }
          ]
        }
        seed_llm_response(lib_dir, 'intro', response)
        ...
        expect(result[:exit_code]).to eq(0)
        expect(result[:stderr]).to include('parent')
        expect(result[:stderr]).to include('2 children')
        expect(result[:arrangement]['beats'].size).to eq(3)
      end
    end

    it 'leaf beat produces prompt with just that beat text' do
      Dir.mktmpdir do |dir|
        lib_dir = make_arrange_library(dir, short_beats: tree_beats, script_format: 'tree')
        response = {
          'beats' => [
            { 'beat_id' => 'end_cta', 'type' => 'cta', 'script_text' => 'Closing words.',
              'clips' => [{ 'source' => 'video.mp4', 't_in' => 1.0, 't_out' => 2.0,
                            'transcript_match' => 'test', 'take_id' => nil, 'notes' => nil }] }
          ]
        }
        seed_llm_response(lib_dir, 'end_cta', response)
        result = run_arrange(dir, short_id: 'end_cta')

        expect(result[:exit_code]).to eq(0)
        expect(result[:arrangement]['beats'].size).to eq(1)
        expect(result[:arrangement]['beats'][0]['beat_id']).to eq('end_cta')
      end
    end
  end
end
