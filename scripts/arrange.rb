...
best_take_hints = ingest['best_take_hints'] || []
unusable_clips = ingest['unusable_clips'] || []

$stderr.puts "  Ingest: #{clip_groups.size} groups, #{clip_groups.sum { |g| (g['clips'] || []).size }} clips"
$stderr.puts "  Unusable: #{unusable_clips.size} clips flagged"
...
enriched_groups = clip_groups.map do |g|
  enriched_clips = (g['clips'] || []).map do |c|
    ...
    entry = { 't' => c['t'], 'source' => c['source'], 'take_variant' => c['take_variant'],
...
prompt = <<~PROMPT
  ...
  ## Best Take Hints
  #{best_take_hints.to_yaml}

  ## Unusable Clips (DO NOT include in arrangement)
  #{unusable_clips.to_yaml}
  ...
  2. **Take selection**: When a clip group has primary + alternate takes, choose the strongest. Put alternates on V2 only when the alternate adds genuine value (different angle, emphasis).
  3. **V2 stacking**: Only use track V2 for: (a) alternate takes worth preserving, (b) cutaway/reaction shots, (c) B-roll overlays. Never put primary narrative on V2.
  4. **Drop logic**: Exclude clips from unusable_clips. Drop filler, false starts, and incomplete thoughts unless they serve a narrative purpose.
  5. **Keep logic**: Every primary clip from the ingest should appear unless explicitly dropped with reasoning in key_decisions.
  ...
  8. **t_in / t_out**: Use the `t` value as t_in. Use the `e` value (if available) as t_out. If `e` is not available, estimate from content.
  9. **Duration**: Estimate total duration from sum of (t_out - t_in) for all V1 clips. Warn if outside target range.
  10. **Narrative roles**: Assign a narrative_role to each clip from: hook, setup, argument, body, evidence, example, quote, anecdote, payoff, resolution, conclusion, transition, bridge.
...
unusable_t_values = unusable_clips.map { |u| u['t'].to_f }
