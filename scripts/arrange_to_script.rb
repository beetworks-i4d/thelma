...
elsif script_parsed['format'] == 'tree' && script_parsed['beats']
  all_beats = script_parsed['beats']
  if short_id.downcase == 'all'
    beats = all_beats
    $stderr.puts "Script format: tree (all #{beats.size} beats)"
  else
    # Resolve by exact id first, then label substring
    target = all_beats.find { |b| b['id'] == short_id }
    target ||= all_beats.find { |b| b['label']&.downcase&.include?(short_id.downcase) }
    if target
      if target['children'] && !target['children'].empty?
        # Parent beat: include parent + children as sub-beats
        children = target['children'].map { |cid| all_beats.find { |b| b['id'] == cid } }.compact
        beats = [target] + children
        $stderr.puts "Script format: tree, parent: #{target['label']} (#{children.size} children)"
      else
        beats = [target]
        $stderr.puts "Script format: tree, leaf: #{target['label']}"
      end
    end
  end
elsif script_parsed['format'] == 'single' && script_parsed['beats']
  # Legacy single format: --short selects a section by label (case-insensitive partial match) or "all"
  if short_id.downcase == 'all'
...
beats_block = beats.each_with_index.map do |b, i|
  role = b['role'] || 'unknown'
  label = b['label'] || b['id'] || "beat_#{i + 1}"
  "Beat #{i + 1} (#{role}: #{label}): #{b['text']}"
end.join("\n\n")
