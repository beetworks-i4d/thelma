...
- Phase 2.5: Pre-Build Sanity Check — review selected candidates before building (main conversation)
...
8. Run Phase 2.5 (pre-build sanity check) — in main conversation (see Phase 2.5 details below)
9. Pass selected storyline(s) + branch assignment into Phase 3 Task agent prompt
10. Run Phase 3 (arrangement + XML) — Task agent builds one output per selected candidate
11. Run Phase 3.5 + Phase 4 (summary + present) in main conversation using final YAML(s)
...
### Phase 2.5: Pre-Build Sanity Check (deep mode, Branch B)

After the user selects candidates in Phase 2, run the sanity check before proceeding to Phase 3. This surfaces candidate content summaries, structural assessments, and unused high-signal warnings so the user can confirm or swap before building.

**Skip conditions:** Fast mode, Branch A (script-locked), or `--skip-sanity-check` flag in `project_config.yaml`.

```bash
ruby scripts/sanity_check.rb <storylines_scored.yaml> <segments_file.yaml> [candidate_ids...]
```

- Pass `segments_deduped.yaml` as the segments file (or `segments_classified.yaml` if dedup wasn't run)
- Pass selected candidate IDs as trailing arguments. If omitted, reviews all ranked candidates.
- Output: `sanity_check.yaml` in same directory as `storylines_scored.yaml`

**After the script runs:**

1. Read `sanity_check.yaml`
2. For each candidate with a `logline_prompt`, read the prompt and generate a 1-2 sentence logline describing what the candidate is ABOUT as content. Write the logline back into the review data.
3. Present the review card to the user:

```
CANDIDATE: short_results_reveal_led
Profile: best_short | Duration: 33s | Combined score: 80
Template: three_item_framework (67% complete)
Coherence: 90 | Issues: none

About: <logline — generated by agent from distilled segments>
Shape: <shape descriptor from script>

Cold open: yes — specific numbers grab without context
Close: yes — identity-durable contrast lands
```

4. Present action options using AskUserQuestion:
   - **Build** — proceed to Phase 3 with this candidate
   - **Swap** — show the next-ranked candidate in same profile (use `next_candidate_id` from review). Re-run sanity check for the swapped candidate: `ruby scripts/sanity_check.rb <scored> <segments> <next_candidate_id>`. If user wants more options, show all candidates by ID for manual selection.
   - **Show unused** — display unused high-signal warnings (if any)
   - **Cancel** — abort build

5. If unused high-signal warnings exist, display them after the candidate review (or on "Show unused"):

```
WARNING: 1 high-signal sequence unused.

Position: 8:34-9:47 (73s, 3 segments)
Distillations:
  - "freelancer hates platform fees"
  - "switched to drop servicing"
  - "first sale within two weeks"
States: vindication(identity), aspiration(mood), competence(identity)

This looks like a complete shorts arc. Review unused or proceed?
```

**Multi-output projects:** Show rollup of all selected candidates, then per-candidate review cards. Unused high-signal is calculated once across the union of all selected candidates' segments.

**Single-output projects:** Sanity check is the default review step — one candidate card + unused warning.
